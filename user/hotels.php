    ...
    foreach ($raw_results as $row) {
        ...
        if ((in_array($row['room_type'], ['双床房', '套房', '大床房', '总统套房', '副总统套房'])) && !empty($row['shared_room_info'])) {
            ...
            if ($existing_room_key !== null) {
                ...
                // 更新房间信息，根据实际入住人员生成
                $personnel_list = implode('、', $room_groups[$existing_room_key]['personnel_names']);
                $room_groups[$existing_room_key]['shared_room_info'] = '房间（' . $personnel_list . '共享）';
            } else {
                // 创建新房间组
                ...
                $room_groups[$room_key] = [
                    ...
                    'ids' => [$row['id']],
                    'shared_room_info' => '房间（' . $row['personnel_name'] . '）',
