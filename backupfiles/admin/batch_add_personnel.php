...
if ($step === 1 && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'return_to_edit') {
        ...
        if (!empty($personnel_data)) {
            $parsed_personnel = json_decode($personnel_data, true);
            // 保留原始输入文本以便重新显示
            $original_input_text = '';
            if (!empty($parsed_personnel)) {
                foreach ($parsed_personnel as $person) {
                    $original_input_text .= $person['name'] . "\t" . $person['id_card'] . "\t" . $person['phone'] . "\t" . $person['gender'] . "\t" . $person['email'] . "\n";
                }
            }
        }
    } else {
